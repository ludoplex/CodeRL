...
def generate_critic_inputs(args, test_case_path, prompt_path, solutions_path, tokenizer, 
    ...
    for solution in solutions:
        solution_str = (
            dsutils.reindent_code(solution)
            if gt_solutions
            else dsutils.reindent_code(solution['code'])
        )
...
def main(args):
    ...
    original_problems = glob.glob(f'{args.test_path}/*')
    ...
    if args.critic_scores: 
        print(f"Total number of samples: {len(all_gts)}")
        acc = (np.array(all_preds) == np.array(all_gts)).sum()/len(all_gts)
        print(f"Error Pred Acc: {acc}")
